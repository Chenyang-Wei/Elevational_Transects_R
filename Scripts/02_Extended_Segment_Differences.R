...
str(transects_TwoDiff_Processed)

# Change the levels of "Type".
levels(transects_TwoDiff_Processed$Type) <- 
  c("VCH", "NDVI")
...
boxplot <- 
  ...
  xlab("Ratio") +
  scale_y_continuous(
    # Features of the first axis (VCH).
    name = "Canopy height difference (m)",
    limits = c(-20, 10),
    
    # Add a second axis (NDVI) and specify its features.
